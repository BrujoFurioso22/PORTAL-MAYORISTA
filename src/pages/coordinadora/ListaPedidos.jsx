...
import { FaSearch, FaFilter, FaEye, FaEdit, FaCheck, FaTimes } from "react-icons/fa";
...
const TableRow = styled.tr`
  ...
  &:hover {
    background-color: ${props => props.theme.colors.border + '40'}; // 40 = 25% opacity
  }
`;
...
const StatusBadge = styled.span`
  ...
  background-color: ${props => {
    switch (props.status) {
      ...
      case 'en_proceso': return props.theme.colors.info + '33';
      case 'en_proceso_observacion': return props.theme.colors.info + '66';
      case 'rechazado': return props.theme.colors.error + '33';
      case 'cancelado_cliente': return props.theme.colors.error + '66';
      case 'completado': return props.theme.colors.success + '33';
      case 'despachado': return props.theme.colors.success + '66';
  ...
  color: ${props => {
    switch (props.status) {
      ...
      case 'en_proceso': case 'en_proceso_observacion': return props.theme.colors.info;
      case 'rechazado': case 'cancelado_cliente': return props.theme.colors.error;
      case 'completado': case 'despachado': return props.theme.colors.success;
    ...
    const loadOrders = async () => {
      ...
      try {
        ...
        setTimeout(() => {
          ...
          const newAddresses = accessibleOrders.filter(order => order.newAddress).length;
  ...
  return (
    <PageContainer>
      ...
      {newAddressesCount > 0 && (
        <AddressAlert>
          <AlertText>
            Hay {newAddressesCount} pedido{newAddressesCount > 1 ? 's' : ''} con direcciones nuevas que requieren confirmación
                    ...
                    <ActionButtons>
                      ...
                      {order.newAddress && (
                        <ActionButton 
                          ...
                          onClick={() => handleConfirmAddress(order.id)}
                          style={{ color: '#f59e0b' }}
                        >
                          <FaCheck />
                        </ActionButton>
